"use strict";
/* tslint:disable */
/* eslint-disable */
/**
 * AI Babe Public API
 * Lorem ipsum
 *
 * The version of the OpenAPI document: 0.0.1
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.instanceOfUserPreferenceModel = instanceOfUserPreferenceModel;
exports.UserPreferenceModelFromJSON = UserPreferenceModelFromJSON;
exports.UserPreferenceModelFromJSONTyped = UserPreferenceModelFromJSONTyped;
exports.UserPreferenceModelToJSON = UserPreferenceModelToJSON;
exports.UserPreferenceModelToJSONTyped = UserPreferenceModelToJSONTyped;
const LLM_1 = require("./LLM");
/**
 * Check if a given object implements the UserPreferenceModel interface.
 */
function instanceOfUserPreferenceModel(value) {
    if (!('userId' in value) || value['userId'] === undefined)
        return false;
    return true;
}
function UserPreferenceModelFromJSON(json) {
    return UserPreferenceModelFromJSONTyped(json, false);
}
function UserPreferenceModelFromJSONTyped(json, ignoreDiscriminator) {
    if (json == null) {
        return json;
    }
    return {
        'createdAt': json['created_at'] == null ? undefined : (new Date(json['created_at'])),
        'updatedAt': json['updated_at'] == null ? undefined : (new Date(json['updated_at'])),
        'id': json['id'] == null ? undefined : json['id'],
        'userId': json['user_id'],
        'displayGirls': json['display_girls'] == null ? undefined : json['display_girls'],
        'displayAnime': json['display_anime'] == null ? undefined : json['display_anime'],
        'displayUnlockWarning': json['display_unlock_warning'] == null ? undefined : json['display_unlock_warning'],
        'usernameUpdated': json['username_updated'] == null ? undefined : json['username_updated'],
        'llmName': json['llm_name'] == null ? undefined : (0, LLM_1.LLMFromJSON)(json['llm_name']),
    };
}
function UserPreferenceModelToJSON(json) {
    return UserPreferenceModelToJSONTyped(json, false);
}
function UserPreferenceModelToJSONTyped(value, ignoreDiscriminator = false) {
    if (value == null) {
        return value;
    }
    return {
        'created_at': value['createdAt'] == null ? undefined : ((value['createdAt']).toISOString()),
        'updated_at': value['updatedAt'] == null ? undefined : ((value['updatedAt']).toISOString()),
        'id': value['id'],
        'user_id': value['userId'],
        'display_girls': value['displayGirls'],
        'display_anime': value['displayAnime'],
        'display_unlock_warning': value['displayUnlockWarning'],
        'username_updated': value['usernameUpdated'],
        'llm_name': (0, LLM_1.LLMToJSON)(value['llmName']),
    };
}
//# sourceMappingURL=UserPreferenceModel.js.map